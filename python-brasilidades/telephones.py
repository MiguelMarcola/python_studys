...
class Tel:
    ...
    def mask_number(self):
        ...
        if response.group(1):
            mask_number = f"Number -> +{response.group(1)} ({response.group(2)}){response.group(3)}-{response.group(4)}"
        else:
            mask_number = f"Number -> +55 ({response.group(2)}){response.group(3)}-{response.group(4)}"
        return mask_number
